...
import  libs.tf_extend.tensors as extend_tensors
from data.preprocessing import tf_image


slim = tf.contrib.slim

def tf_image_whitened(image, means=cfgs.PIXEL_MEAN):
...
def tf_image_unwhitened(image, means=cfgs.PIXEL_MEAN, to_int=True):
...
def np_image_unwhitened(image, means=(cfgs.PIXEL_MEAN), to_int=True):
    """Re-convert to original image distribution, and convert to int if
...
def distorted_bounding_box_crop(image,
    ...
    with tf.name_scope(scope, 'distorted_bounding_box_crop', [image, bboxes]):
        ...
        bboxes = bboxes_utils.bboxes_resize(distort_bbox, bboxes)
        labels, bboxes = bboxes_utils.bboxes_filter_overlap(labels, bboxes,
                                                   threshold=cfgs.BBOX_CROP_OVERLAP,
    ...
    with tf.name_scope(scope, 'ssd_preprocessing_train', [image, labels, bboxes]):
        ...
        dst_image, labels, bboxes, distort_bbox = \
            distorted_bounding_box_crop(image, labels, bboxes,
                                        min_object_covered=cfgs.MIN_OBJECT_COVERED,
                                        aspect_ratio_range=cfgs.CROP_RATIO_RANGE)
        ...
        image = dst_image * 255.
        image = tf_image_whitened(image, cfgs.PIXEL_MEAN)
        # Image data format.
        if data_format == 'NCHW':
...
def preprocess_for_eval(image, labels, bboxes,
                        out_shape=cfgs.EVAL_SIZE, data_format='NHWC',
                        difficults=None, resize=cfgs.Resize.WARP_RESIZE,
                        scope='ssd_preprocessing_train'):
    ...
    with tf.name_scope(scope):
        ...
        if resize == cfgs.Resize.NONE:
            # No resizing...
            pass
        elif resize == cfgs.Resize.CENTRAL_CROP:
            # Central cropping of the image.
            image, bboxes = tf_image.resize_image_bboxes_with_crop_or_pad(
                image, bboxes, out_shape[0], out_shape[1])
        elif resize == cfgs.Resize.PAD_AND_RESIZE:
            ...
            image, bboxes = tf_image.resize_image_bboxes_with_crop_or_pad(
                image, bboxes, out_shape[0], out_shape[1])
        elif resize == cfgs.Resize.WARP_RESIZE:
