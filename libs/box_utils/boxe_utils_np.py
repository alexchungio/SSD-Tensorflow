...
def bboxes_recover(bboxes, image):
    assert len(image.shape) == 3;
    height, width = image.shape[0], image.shape[1]
    bboxes = np.copy(bboxes)
    bboxes = np.transpose(bboxes)

    y_min = np.multiply(bboxes[0], height)
    x_min = np.multiply(bboxes[1], width)
    y_max = np.multiply(bboxes[2], height)
    x_max = np.multiply(bboxes[3], width)
    bboxes = np.stack([x_min, y_min, x_max, y_max], axis=0)
