...
# -*- coding: utf-8 -*-
#------------------------------------------------------
# @ File       : draw_box_in_tensor.py
# @ Description:  
# @ Author     : Alex Chung
# @ Contact    : yonganzhong@outlook.com
# @ License    : Copyright (c) 2017-2018
# @ Time       : 2020/8/4 下午7:18
# @ Software   : PyCharm
#-------------------------------------------------------
...
def only_draw_boxes(img_batch, boxes):
    ...
    labels = tf.ones(shape=(tf.shape(boxes)[0], ), dtype=tf.int32) * draw_box_in_image.ONLY_DRAW_BOXES
    scores = tf.zeros_like(labels, dtype=tf.float32)
    img_tensor_with_boxes = tf.py_func(draw_box_in_image.draw_boxes_with_label_and_scores,
...
def draw_boxes_with_scores(img_batch, boxes, scores):
    ...
    img_tensor = tf.squeeze(img_batch, 0)
    img_tensor = tf.cast(img_tensor, tf.float32)
    labels = tf.ones(shape=(tf.shape(boxes)[0],), dtype=tf.int32) * draw_box_in_image.ONLY_DRAW_BOXES_WITH_SCORES
    img_tensor_with_boxes = tf.py_func(draw_box_in_image.draw_boxes_with_label_and_scores,
                                       inp=[img_tensor, boxes, labels, scores],
                                       Tout=[tf.uint8])
