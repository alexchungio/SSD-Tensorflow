...
def bboxes_nms(scores, bboxes, nms_threshold=0.5, keep_top_k=200, scope=None):
    ...
    with tf.name_scope(scope, 'bboxes_nms_single', [scores, bboxes]):
        ...
        # Pad results.
        scores = extend_tensors.pad_axis(scores, 0, keep_top_k, axis=0)
        bboxes = extend_tensors.pad_axis(bboxes, 0, keep_top_k, axis=0)
        return scores, bboxes
...
def bboxes_jaccard(bbox_ref, bboxes, name=None):
    ...
    with tf.name_scope(name, 'bboxes_jaccard'):
        ...
        jaccard = extend_math.safe_divide(inter_vol, union_vol, 'jaccard')
        return jaccard
...
def bboxes_intersection(bbox_ref, bboxes, name=None):
    ...
    with tf.name_scope(name, 'bboxes_intersection'):
        ...
        scores = extend_math.safe_divide(inter_vol, bboxes_vol, 'intersection')
        return scores
